...
Here we look at the fold-change data used in the Pre Non-linear Transformation/Input files. This allows us to see positive and negative functional effects across the landscapes.
...
all_ratios = lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="ratio_export.csv", recursive = T)), read_csv, skip = 1, show_col_types=F, col_names = c("id", "effect", "muts", "cv", "colors"))

all_ratios_id = str_remove_all(list.files(path = "Pre Non-linear Transformation/Output", pattern="ratio_export.csv", recursive = T), "/ratio_export.csv")
...
Filtering ratio data to only include genotypes with 2+ mutations. Pre Non-linear Transformation/Outputting length of vector.
...
wt_rel_df = do.call("rbind", lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="*wt_rel.csv", recursive = T)), read_csv, show_col_types=F, col_names = c("positions", "identity", "mut", "effects", "enzyme", "type", "cond")))
...
All Pre Non-linear Transformation/Outputs show position/combination numbers in each category, followed by the total sum of all probed positions/combinations at that order, and finally percentages of each category
