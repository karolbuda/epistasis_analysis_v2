---
title: "Pre Non-linear Transformation Output"
Pre Non-linear Transformation/Output: html_document
---
...
This is the curated version of data, which only includes analyses presented in the manuscript. The order in this markdown file follows the order as seen in the manuscript.

## Loading necessary data

Some data in the exploratory analysis was uploaded and used early on. Though this analysis was not included in the paper, some of the variable names for the datasets are still used. For simplicity, I upload the data here to the variables, but do not provide analysis so as not to complicate this markdown file. If you'd like to explore more deeply, see ```statistical_epistasis_analysis.Rmd```

```{r, loading, message=F, echo=FALSE}
fit_land = do.call("rbind", lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="observed_values.csv", recursive = T)), read_csv, skip = 1, show_col_types=F, col_names = c("id", "effect", "muts", "junk1", "junk2", "enz")))
...
fit_land_ids = rep(str_remove_all(list.files(path = "Pre Non-linear Transformation/Output", pattern="observed_values.csv", recursive = T), "/observed_values.csv"), fit_land_counts)
...
This part is done by the ```epistasis_model_bulk_general.R``` script.

### Idiosyncrasy in single mutational effects (SMEs)
...
d = do.call("rbind", lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="*2d_box.csv", recursive = T)), read_csv, show_col_types=F, col_names = c("positions", "identity", "mut", "effects", "enzyme", "type", "cond")))
...
idio_d = do.call("rbind", lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="idio_df", recursive = T)), read_csv, show_col_types=F))

traj_col = sapply(str_split(list.files(path = "Pre Non-linear Transformation/Output", pattern="idio_df", recursive = T), "/"),"[[",1)

traj_col_len = sapply(lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="idio_df", recursive = T)), read_csv, show_col_types=F), dim)[1,]
...
First we upload all SME and EE data for sign changing

```{r,sign-load,message=F,echo=F}
higher_order_list = list.files(path = "Pre Non-linear Transformation/Output", pattern="higher_box", recursive = T)
...
for(higher_file in 1:length(higher_order_list)){
  ...
  appending_file = read_csv(paste0("Pre Non-linear Transformation/Output/", higher_order_list[higher_file]), show_col_types=F)
...
Then, we assigning positive, neutral, negative, negative-neutral, positive-neutral, and negative-positive 'types' based on log10 1.5-fold threshold to every Order
...
If one chooses to probe the variable ```higher_all_sign_check```, it will show position/combination numbers in each category, followed by the total sum of all probed positions/combinations at that order, and finally percentages of each category
...
Purpose of this analysis was to determine the difference in SMEs between the WT-background contribution of a given position/combination and the mean contribution of a given position or combination

First, we load the spread of every positional contribution vs mean differences for SMEs
...
As well as the devation of every single SME and EE from the mean, instead of just the wt background. This corresponds to extended data table 4.
...
### Idiosyncrasy in epistatic effects
...
#ggsave("fig_sup_2A_1.svg", idio_d_plot_2, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
idio_d_plot_3 = idio_d %>%
  ...
  ggplot(aes(x = idiosync)) +
  geom_histogram(bins = 30, fill = "#edae49", color = "black", lwd = 0.1) +
  geom_vline(xintercept = log10(1.5), lty = 2, lwd = 0.2) +
  geom_vline(xintercept = log10(2), lty = 2, lwd = 0.2) +
  geom_vline(xintercept = log10(5), lty = 2, lwd = 0.2) +
  geom_vline(xintercept = log10(10), lty = 2, lwd = 0.2) +
...
#ggsave("fig_sup_2A_2.svg", idio_d_plot_3, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
#ggsave("fig_sup_2A_3.svg", idio_d_plot_4, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
#ggsave("fig_sup_2C_1.svg", devs_df_wt_2, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
#ggsave("fig_sup_2C_2.svg", devs_df_wt_3, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
#ggsave("fig_sup_2C_3.svg", devs_df_wt_4, width = 180/3, height = 247/4, dpi = 300, units = "mm")
```
...
all_mae = lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="aic_*", recursive = T)), read_csv, skip = 1, show_col_types=F, col_names = c("MAE", "Last MAE", "Step", "Enzyme", "Linear MAE", "Linear Last MAE"))

for(element in 1:length(all_mae)){
  all_mae[[element]]$ID = str_split(list.files(path = "Pre Non-linear Transformation/Output", pattern="aic_*", recursive = T)[element], "/")[[1]][1]
}
...
mae_ggplot
ggsave("fig_3A.svg", mae_ggplot, width = 180/2, height = 247/4, dpi = 300, units = "mm")
```
...
all_traj = do.call("rbind", lapply(paste0("Pre Non-linear Transformation/Output/", list.files(path = "Pre Non-linear Transformation/Output", pattern="traj_epi_*", recursive = T)), read_csv, skip = 1, show_col_types=F, col_names = c("genotype", "avg", "pos", "mutations", "likely", "enzyme_name", "measure_type", "cond")))
...
for(i in 1:length(adaptive_trajectories)) {
  ...
  d = read_csv(paste0("Pre Non-linear Transformation/Output/", adaptive_trajectories[i], "/", file), show_col_types=F)
...
Take all the Pre Non-linear Transformation/Input files, attach their unique_id, find the ones with replicates, then look at the median of 2*SD (~95% spread around measurement mean) for all measurements to decided on an approriate significance threshold

```{r}
file_vector = str_remove(list.files(path = "Pre Non-linear Transformation/Input", pattern="*.csv", recursive = T), ".csv")

error_set = lapply(paste0("Pre Non-linear Transformation/Input/", list.files(path = "Pre Non-linear Transformation/Input", pattern="*.csv", recursive = T)), read_csv, skip = 5, col_names = F, col_select = 1:2, show_col_types=F)
