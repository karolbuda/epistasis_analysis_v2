...
for(element in 1:length(all_mae)){
  all_mae[[element]]$ID = str_split(list.files(path = "trans_Output", pattern="aic_*", recursive = T)[element], "/")[[1]][1]
}
...
all_traj = do.call("rbind", lapply(paste0("trans_Output/", list.files(path = "trans_Output", pattern="traj_epi_*", recursive = T)), read_csv, skip = 1, show_col_types=F, col_names = c("genotype", "avg", "pos", "mutations", "likely", "enzyme_name", "measure_type", "cond")))
...
for(i in 1:length(adaptive_trajectories)) {
  ...
  d = read_csv(paste0("trans_Output/", adaptive_trajectories[i], "/", file), show_col_types=F)
