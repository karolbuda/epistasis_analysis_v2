...
knitr::opts_knit$set(root.dir = "/Users/karolbuda/OneDrive - UBC/PhD/Epistasis_Lit/__VersionCtrl__/epistasis_analysis_v2/Input")
...
```
We then have to change to the output directory

```{r, setup, include = F}
knitr::opts_knit$set(root.dir = "/Users/karolbuda/OneDrive - UBC/PhD/Epistasis_Lit/__VersionCtrl__/epistasis_analysis_v2/Output")
```
...
print(paste0(sum(idio_first > log10(1.5)), "/", length(idio_first)))
print(paste0(sum(idio_first > log10(2)), "/", length(idio_first)))
print(paste0(sum(idio_first > log10(5)), "/", length(idio_first)))
print(paste0(sum(idio_first > log10(10)), "/", length(idio_first)))
...
```
What percentage of positions remain significantly heterogeneous (1.5-, 2-, 5-, and 10-fold) at these orders?
...
devs_df_wt %>%
  filter(muts == 3) %>%
  ggplot(aes(x = devs)) +
  geom_histogram(binwidth=0.1, fill = "#edae49", color = "black", lwd = 0.1) +
  geom_vline(xintercept = log10(1.5), lty = 2, lwd = 1) +
  geom_vline(xintercept = log10(2), lty = 2, lwd = 1) +
  geom_vline(xintercept = log10(5), lty = 2, lwd = 1) +
  geom_vline(xintercept = log10(10), lty = 2, lwd = 1) +
...
```{r,message=F}
all_traj = do.call("rbind", lapply(list.files(pattern="traj_epi_*", recursive = T), read_csv, skip = 1, show_col_types=F, col_names = c("genotype", "avg", "pos", "mutations", "likely", "enzyme_name", "measure_type", "cond")))
...
for(i in 1:length(adaptive_trajectories)) {
  ...
  d = read_csv(paste0(adaptive_trajectories[i], "/", file), show_col_types=F)
...
write.csv(plotting_d %>%
  pivot_wider(names_from = name, values_from = value) , "/Users/karolbuda/OneDrive - UBC/PhD/Epistasis_Lit/Bulk/preds.csv")
```
...
ggplot(higher_df_diminish, aes(x = start_effect, y = avg)) +
  ...
  theme_classic() 
```
What about only looking at the best step from each starting point?
